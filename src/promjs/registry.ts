...
interface RegistryItem<Type extends CollectorType> {
  [key: string]: {
    type: Type;
    help: string;
    instance: CollectorForType<Type>;
  };
}
...
export class Registry {
  ...
  private validateInput(
    ...
    if (this.data[type][name]) {
      throw new Error(
        `A metric with the name '${name}' already exists for type '${type}'`,
      );
    }
  ...
  reset(): this {
    Object.values(this.data).map((m) =>
      Object.values(m).map(({ instance }) => instance.resetAll()),
    );
    return this;
  }
  ...
  get(type: CollectorType, name: string): Collector<any> | undefined {
    const registryItems =
      type != null ? [this.data[type]] : Object.values(this.data);
